...
const targetStart = '{isSavingFirm ? "Deploying Updates..." : "Sync Firm Profile"}';
const index = content.lastIndexOf(targetStart);

if (index !== -1) {
    // Find the start of the <Button block
    const buttonStart = '<div className="flex items-center gap-4 pt-6">';
    const bIndex = content.lastIndexOf(buttonStart);

    if (bIndex !== -1) {
        const preamble = content.substring(0, bIndex + buttonStart.length);
        const newClosing = `
                        <Button
...
`;
        ...
        console.log("Verified Save Complete");
    } else {
        console.log("Preamble not found");
    }
} else {

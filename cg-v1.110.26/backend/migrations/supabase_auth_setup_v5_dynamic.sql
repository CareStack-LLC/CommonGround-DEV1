-- ============================================================================
-- Supabase User Setup for Partner Admins (Run in SQL Editor) - v5
...
-- 2. Dynamically fetches Partner IDs (no hardcoded UUIDs)
-- 3. Removes 'role' from public.users table
-- ============================================================================
...
BEGIN
  ...
  -- 1. Fetch Partner IDs dynamically
  SELECT id INTO jenesse_partner_id FROM partners WHERE partner_slug = 'jenesse-center';
  SELECT id INTO interval_partner_id FROM partners WHERE partner_slug = 'interval-house-lb';

  -- Validate partners exist
  IF jenesse_partner_id IS NULL THEN
    RAISE EXCEPTION 'Partner jenesse-center not found';
  END IF;
  
  IF interval_partner_id IS NULL THEN
    RAISE EXCEPTION 'Partner interval-house-lb not found';
  END IF;

  -- 2. Cleanup existing records (to avoid conflicts if re-running)
  ...
  WHERE partner_id::text IN (jenesse_partner_id::text, interval_partner_id::text);

  -- 3. Insert Jenesse Admin
  ...
  -- Explicitly provide ID, created_at, updated_at
  ...
  INSERT INTO public.partner_staff (
    ...
    gen_random_uuid(), -- Explicitly generate ID
    interval_partner_id, -- Dynamic ID
    interval_user_id::text,
    'admin',
    now_timestamp,
    now_timestamp

...
class ComplianceReportService:
    ...
    async def generate_pdf_bytes(
        ...
        Uses a lightweight HTML-to-text approach for the MVP;
        drop in ReportLab or WeasyPrint for production-quality PDFs.
        """
        import io
        import csv as _csv  # unused, just to avoid shadowing

        lines: list[str] = []
        lines.append("=" * 72)
        lines.append("COMMONGROUND™ COMPLIANCE REPORT")
        lines.append("=" * 72)
        lines.append("")
        lines.append(f"Report Title:  {report.title or 'Compliance Summary'}")
        lines.append(f"Case ID:       {report.family_file_id}")
        lines.append(f"Generated:     {datetime.utcnow().strftime('%B %d, %Y at %I:%M %p UTC')}")
        lines.append(f"Format:        {report.export_format or 'PDF'}")
        lines.append(f"Period:        {report_data['period']['start'] or 'All time'} → "
                      f"{report_data['period']['end'] or 'Present'}")
        lines.append("")

        # ARIA Safety Summary
        aria = report_data.get("aria_stats", {})
        lines.append("-" * 40)
        lines.append("ARIA™ SAFETY SHIELD SUMMARY")
        lines.append("-" * 40)
        lines.append(f"  Total flags raised:     {aria.get('total_flags', 0)}")
        sev = aria.get("severity_counts", {})
        lines.append(f"  Severe:  {sev.get('severe', 0)}   High: {sev.get('high', 0)}   "
                      f"Medium: {sev.get('medium', 0)}   Low: {sev.get('low', 0)}")
        lines.append(f"  Resolution rate:        {aria.get('resolution_rate', 0):.1f}%")
        top = aria.get("top_categories", {})
        if top:
            lines.append(f"  Top categories:         {', '.join(top.keys())}")
        lines.append("")

        # Exchange Compliance
        exc = report_data.get("exchange_compliance", {})
        lines.append("-" * 40)
        lines.append("CUSTODY EXCHANGE COMPLIANCE")
        lines.append("-" * 40)
        lines.append(f"  Total scheduled:        {exc.get('total_exchanges', 0)}")
        lines.append(f"  Completed:              {exc.get('completed', 0)}")
        lines.append(f"  Missed:                 {exc.get('missed', 0)}")
        lines.append(f"  On-time rate:           {exc.get('on_time_rate', 0):.1f}%")
        lines.append(f"  GPS-verified rate:      {exc.get('gps_verified_rate', 0):.1f}%")
        lines.append("")
        ...
        lines.append("-" * 40)
        lines.append(f"  Total messages:         {comm.get('total_messages', 0)}")
        lines.append(f"  Avg response time:      {comm.get('avg_response_time_hours', 0):.1f} hrs")
        lines.append(f"  Sentiment score:        {comm.get('sentiment_score', 0)}/100")
        lines.append("")
        ...
        if report.signature_line:
            lines.append("")
            lines.append("_" * 40)
            lines.append(f"Prepared by: {report.signature_line}")
            lines.append("")
        ...
        lines.append("This report was generated by CommonGround™ and has not been altered.")
        lines.append("─" * 72)

        final_content = "\n".join(lines)
        pdf_bytes = final_content.encode("utf-8")

            ...
            <CardContent className="space-y-6">
                ...
                {currentSession && (
                    <div className="bg-muted p-4 rounded-lg border border-border flex items-center space-x-4">
                        ...
                        <div className="bg-card p-2 rounded-full shadow-sm">
                            <Clock className="h-6 w-6 text-cg-slate" />
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Current Custody Status</p>
                            <h3 className="text-xl font-bold text-foreground">
                                {activeParentName}
                                <span className="ml-2 text-cg-slate font-mono">{currentDurationText}</span>
                            </h3>
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-end">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-baseline space-x-2">
                                ...
                                <span className={`text-xs font-medium ${stats.parent_a.variance > 0 ? 'text-cg-sage-dark' : 'text-cg-error'}`}>
                        ...
                        <div className="space-y-1 text-right">
                            ...
                            <div className="flex items-baseline space-x-2 justify-end">
                                <span className={`text-xs font-medium ${stats.parent_b.variance > 0 ? 'text-cg-sage-dark' : 'text-cg-error'}`}>
                    ...
                    <div className="h-4 w-full bg-muted rounded-full overflow-hidden flex">
                        <div
                            className="bg-cg-slate h-full transition-all duration-1000 ease-out"
                            style={{ width: `${stats.parent_a.percentage}%` }}
                        />
                        <div
                            className="bg-cg-slate-light h-full transition-all duration-1000 ease-out"
                        ...
                        {[...sessions].reverse().slice(0, 5).map((session, idx) => (
                            <div key={idx} className="flex justify-between items-center text-sm p-2 hover:bg-muted rounded">
                                <div className="flex items-center space-x-2">
                                    <div className={`w-2 h-2 rounded-full ${session.parent_id === stats.parent_a.user_id ? 'bg-cg-slate' : 'bg-cg-slate-light'}`} />

...
const STATUS_CONFIG: Record<string, { icon: React.ElementType; color: string; dot: string }> = {
  ...
  in_progress: { icon: Loader2, color: 'text-[#4BA8C8]', dot: 'bg-[#4BA8C8]' },
};
...
const ENV_BADGES: Record<string, string> = {
  ...
  preview: 'bg-[#4BA8C8]/15 text-[#4BA8C8] border-[#4BA8C8]/30',
};
...
export function DeploymentTimeline({ deployments, loading }: DeploymentTimelineProps) {
  ...
  if (!deployments.length) {
    return <div className="text-center py-8 text-sm text-[#6B8A9A]">No deployments recorded yet</div>;
  }

  return (
    <div className="relative">
      {/* Timeline line */}
      <div className="absolute left-4 top-0 bottom-0 w-px bg-[#2D6A8F]/30" />
        ...
        {deployments.map((deploy) => {
          ...
          return (
            <div key={deploy.id} className="relative pl-10 py-2 group">
              ...
              <div className="bg-[#1A3648]/40 border border-[#2D6A8F]/15 rounded-lg px-3 py-2.5 hover:border-[#2D6A8F]/30 transition-colors">

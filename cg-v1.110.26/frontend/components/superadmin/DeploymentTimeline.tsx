        ...
        {deployments.map((deploy) => {
          ...
          return (
            <div key={deploy.id} className="relative pl-10 py-2 group">
              {/* Timeline dot */}
              <div className={`absolute left-3 top-4 w-3 h-3 rounded-full ${config.dot} border-2 border-cg-slate-deep`} />

              <div className="bg-cg-slate-deep/40 border border-cg-slate/15 rounded-lg px-3 py-2.5 hover:border-cg-slate/30 transition-colors">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-cg-slate-strong ml-auto">{timeAgo(deploy.deployed_at)}</span>
                </div>

                {deploy.commit_message && (
                  <div className="text-xs text-cg-slate-muted line-clamp-1 mb-1">{deploy.commit_message}</div>
                )}

                <div className="flex items-center gap-3 text-[10px] text-cg-slate-strong">
                  {deploy.commit_sha && (

...
export function ARIAIntervention({
  ...
  const getLevelConfig = (level: string, isBlocked: boolean) => {
    ...
    switch (level) {
      case 'green':
        return {
          bg: 'bg-gradient-to-br from-cg-sage-subtle to-white dark:from-foreground/30 dark:to-card',
          border: 'border-cg-sage-subtle dark:border-foreground/60',
          iconBg: 'bg-cg-sage-subtle dark:bg-foreground/40',
          iconColor: 'text-cg-sage',
          icon: <CheckCircle className="h-6 w-6 text-cg-sage" />,
          title: 'Communication Verified',
          subtitle: 'Your message maintains a constructive tone.',
          btnPrimary: 'bg-cg-sage-dark hover:bg-cg-sage-dark text-white',
        };
      case 'yellow':
        return {
          bg: 'bg-gradient-to-br from-cg-amber-subtle to-white dark:from-foreground/30 dark:to-card',
          border: 'border-cg-amber-subtle dark:border-foreground/60',
          iconBg: 'bg-cg-amber-subtle dark:bg-foreground/40',
  ...
  const getCategoryColor = (category: string) => {
    ...
    if (['hate_speech', 'sexual_harassment', 'threatening'].includes(cat)) return 'bg-cg-error';
    if (['custody_weaponization', 'financial_coercion', 'hostility'].includes(cat)) return 'bg-cg-amber';
    return 'bg-cg-amber';
  };
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex-shrink-0">
            <div className={`w-14 h-14 rounded-2xl ${config.iconBg} flex items-center justify-center shadow-sm`}>
              ...
              {analysis.block_send ? (
                <Shield className="h-7 w-7 text-cg-error" />
              ) : (
                config.icon
              )}
            </div>

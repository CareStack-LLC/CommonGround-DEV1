...
export default function AgreementUploadPage() {
  ...
  const getStatusBadge = (status: string) => {
    const colors: Record<string, string> = {
      pending: "bg-cg-amber-subtle text-cg-amber-dark",
      processing: "bg-cg-slate-subtle text-cg-slate-dark",
      completed: "bg-cg-sage-subtle text-cg-sage-dark",
      failed: "bg-cg-error-subtle text-cg-error-dark",
      needs_review: "bg-cg-amber-subtle text-cg-amber-dark",
    };
    return (
  ...
  const getConfidenceBadge = (confidence: number | null) => {
    ...
    const color = percent >= 80 ? "text-cg-sage-dark" : percent >= 60 ? "text-cg-amber-dark" : "text-cg-error";
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {uploadProgress && (
                <div className="bg-cg-slate-subtle border border-cg-slate-subtle text-cg-slate-dark px-4 py-3 rounded">
                  {uploadProgress}
                </div>
              )}
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {order.requires_review && (
                          <span className="px-1.5 py-0.5 bg-cg-amber-subtle text-cg-amber-dark text-xs rounded">
                            Review
                          </span>
                        )}
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {selectedOrder.order.requires_review ? (
                      ...
                      <span className="px-2 py-1 bg-cg-sage-subtle text-cg-sage-dark text-sm rounded">
                        Reviewed
                      </span>
                    )}
                  </div>
                        ...
                        {selectedOrder.visitation_schedules.map((schedule) => (
                          <div key={schedule.id} className="bg-slate-50 rounded-lg p-3">
                            <div className="flex items-center justify-between">
                              ...
                              {schedule.is_virtual && (
                                <span className="px-1.5 py-0.5 bg-cg-slate-subtle text-cg-slate-dark text-xs rounded">
                                  Virtual
                                </span>
                              )}
                            </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedOrder.order.has_abuse_allegations && (
                      <span className="px-2 py-1 bg-cg-error-subtle text-cg-error-dark text-xs rounded">
                        Abuse Allegations
                      </span>
                    )}
                    ...
                    {selectedOrder.order.travel_restriction_state && (
                      <span className="px-2 py-1 bg-cg-amber-subtle text-cg-amber-dark text-xs rounded">
                        Travel Restricted
                      </span>
                    )}
                    {selectedOrder.order.mediation_required && (
                      <span className="px-2 py-1 bg-cg-slate-subtle text-cg-slate-dark text-xs rounded">
                        Mediation Required
                      </span>
                    )}
                  ...
                  {selectedOrder.order.extraction_notes && (
                    <div className="bg-cg-amber-subtle border border-cg-amber-subtle rounded-lg p-3">
                      <div className="text-xs text-cg-amber-dark font-medium uppercase mb-1">
                        Extraction Notes
                      </div>

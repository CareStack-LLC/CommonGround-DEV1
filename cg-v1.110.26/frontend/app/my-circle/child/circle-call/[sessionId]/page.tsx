...
function CircleCallContent() {
  ...
  function loadCallSession() {
    try {
      ...
      if (userStr) {
        ...
        setChildUserName(userData.childName || session.childName);
      } else {
        setChildUserName(session.childName);
      }
  ...
  async function initializeCall() {
    ...
    try {
      ...
      console.log('[Circle Call] Creating Daily.co call object...');
      ...
      call.on('joined-meeting', () => {
        console.log('[Circle Call] Joined meeting');
      ...
      call.on('participant-joined', () => {
        console.log('[Circle Call] Participant joined');
        updateParticipants(call.participants());
      });

      call.on('participant-left', () => {
        console.log('[Circle Call] Participant left');
        updateParticipants(call.participants());
      });
      ...
      call.on('left-meeting', () => {
        console.log('[Circle Call] Left meeting');
        setIsCallJoined(false);
        handleEndCall();
      });

      call.on('error', (event) => {
        console.error('[Circle Call] Daily.co error:', event);
        setError('Video call error occurred');
        setIsJoiningCall(false);
      });

      // Join the call
      console.log('[Circle Call] Joining room...', { roomUrl: callSession.roomUrl });
      await call.join({
      ...
      console.log('[Circle Call] Join completed');
    } catch (err) {
      console.error('[Circle Call] Error initializing call:', err);
  ...
  async function handleLeaveCall() {
    try {
      ...
      if (sessionId) {
        await circleCallsAPI.endCall(sessionId, 'child').catch(console.error);
      }

      handleEndCall();
    } catch (err) {
      console.error('[Circle Call] Error leaving call:', err);
      handleEndCall();
    }
  }
  ...
  function handleGoBack() {
    ...
    localStorage.removeItem('circle_call_session');
    router.push('/my-circle/child/circle-contacts');
  }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-background via-white to-[#F5F9F9] flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="h-16 w-16 animate-spin mx-auto mb-4 text-[#2C5F5D]" />
          <p className="text-xl font-bold text-foreground">Connecting your call...</p>
  ...
  if (error || !callSession) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-background via-white to-[#F5F9F9] flex items-center justify-center p-4">
  ...
  return (
    <div className="flex h-screen bg-gradient-to-b from-background via-white to-[#F5F9F9]">
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
        <header className="bg-white/90 backdrop-blur-sm border-b border-gray-200 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 text-[#2C5F5D]">
                  <Shield className="h-3 w-3" /> Protected by ARIA
                </span>
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 relative overflow-hidden">
          {!isCallJoined ? (
            <div className="h-full bg-gradient-to-br from-[#2C5F5D] to-cg-amber/80 flex items-center justify-center">
              <div className="text-center">
        ...
        {/* Controls Bar */}
        <div className="bg-[#2C5F5D] px-4 py-4 shadow-lg">
    ...
    return (
      <div className="relative h-full w-full bg-gray-800">
        {participant.videoOn && participant.videoTrack ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#2C5F5D] to-cg-amber">
    ...
    return (
      <div className="relative h-full w-full bg-gray-900">
        {participant.videoOn && participant.videoTrack ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#2C5F5D] to-cg-amber/80">
...
export default function CircleCallPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-gradient-to-b from-background via-white to-[#F5F9F9] flex items-center justify-center">

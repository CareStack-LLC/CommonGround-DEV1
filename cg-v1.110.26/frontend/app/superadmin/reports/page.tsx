        ...
        <>
          ...
          {weeklyLoading && !weeklyReport && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {Array.from({ length: 8 }).map((_, i) => <div key={i} className="animate-pulse bg-zinc-800/60 rounded-lg h-28" />)}
            </div>
          )}
          ...
          {!weeklyReport && !weeklyLoading && (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-zinc-600 text-xs">Click &ldquo;Generate Now&rdquo; to create the weekly platform snapshot</p>
            </div>
          )}
        </>
      )}

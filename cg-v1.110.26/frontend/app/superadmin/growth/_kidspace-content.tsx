      ...
      {tab === 'overview' && (
        <div className="space-y-4">
          ...
          <div className="bg-zinc-900/50 border border-zinc-800/60 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-zinc-300 mb-4">Daily Usage Trends (30 days)</h2>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            ...
            <div className="bg-zinc-900/50 border border-zinc-800/60 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold text-zinc-300">Avg Session Duration</h2>
              </div>
              ...
              ) : <p className="text-zinc-600 text-sm text-center py-10">No session data</p>}
            </div>
          </div>
        </div>
      )}
                  ...
                  <div key={i} className="flex items-center gap-3 py-2.5 border-b border-zinc-800/30 last:border-0 hover:bg-zinc-800/20 transition-colors px-2 rounded">
                    <span className="w-6 text-center">
                      {i === 0 ? (
                        ...
                        <span className="text-xs font-bold text-zinc-600">#{item.rank}</span>
                      )}
                    </span>
          ...
          {data?.session_averages?.length ? (
            <div className="bg-zinc-900/50 border border-zinc-800/60 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-800">
                <h2 className="text-sm font-semibold text-zinc-200">Session Type Breakdown</h2>
              </div>
                ...
                <tbody>
                  {data.session_averages.map((sa, i) => (
                    <tr key={i} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                      <td className="px-5 py-3 text-zinc-200 capitalize">{sa.type.replace('_', ' ')}</td>
                      <td className="px-5 py-3 text-right text-zinc-300">{formatNumber(sa.total_sessions)}</td>
                      <td className="px-5 py-3 text-right text-zinc-300">{sa.avg_minutes} min</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <div className="flex items-center justify-between">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-0.5">Usage stats, content leaderboards &amp; session details</p>
      </div>

    ...
    <div className="min-h-screen bg-[#162D3A] text-[#D0E4EC]">
      <header className="sticky top-0 z-50 h-14 border-b border-[#2D6A8F]/20 bg-[#162D3A]/95 backdrop-blur-md flex items-center px-4 lg:px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="hidden md:inline-flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-[#0F2533]/60 border border-[#2D6A8F]/20 hover:border-[#2D6A8F]/50 text-xs text-[#8AACBC] hover:text-white transition-colors"
            title="Open command palette (Cmd+K)"
          >
            <Search className="w-3 h-3" />
            <span>Search…</span>
            <kbd className="ml-1 px-1 rounded bg-[#2D6A8F]/20 border border-[#2D6A8F]/30 font-mono text-[10px]">⌘K</kbd>
          </button>
      ...
      <div className="flex">
        ...
        <aside className={`
          ...
          w-56
          bg-[#1E3A4A] border-r border-[#2D6A8F]/20
        ...
        `}>
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {collapsed && (
                          <span className="hidden lg:block absolute left-full top-1/2 -translate-y-1/2 ml-2 whitespace-nowrap bg-[#0F2533] border border-[#2D6A8F]/30 text-xs text-white px-2 py-1 rounded opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-50">
                            {item.label}
                          </span>
                        )}
                      </a>
          ...
          <div className="border-t border-[#2D6A8F]/20">
            {/* Collapse toggle — desktop only */}
            <button
              onClick={() => setCollapsed((c) => !c)}
              className="hidden lg:flex w-full items-center justify-center gap-2 px-3 py-2 text-xs text-[#6B8A9A] hover:text-white hover:bg-[#2D6A8F]/20 transition-colors"
            ...
            <a
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-[#6B8A9A] hover:text-white hover:bg-[#2D6A8F]/20 transition-colors ${collapsed ? 'lg:justify-center' : ''}`}

        ...
        <div className="bg-[#1A3648]/60 border border-[#2D6A8F]/20 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={data.sprints}>
              ...
              <XAxis dataKey="name" stroke="#4A6E7F" tick={{ fill: '#6B8A9A', fontSize: 10 }} />
              <YAxis stroke="#4A6E7F" tick={{ fill: '#6B8A9A', fontSize: 10 }} />
              <Tooltip contentStyle={RECHARTS_TOOLTIP} />
              <Bar dataKey="completed" fill="#3DAA8A" radius={[4, 4, 0, 0]} name="Completed" />
              <Bar dataKey="planned" fill="#2D6A8F" radius={[4, 4, 0, 0]} name="Planned" opacity={0.5} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
        ...
        <div className="bg-[#1A3648]/60 border border-[#2D6A8F]/20 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={trends.daily}>
              ...
              <YAxis stroke="#4A6E7F" tick={{ fill: '#6B8A9A', fontSize: 10 }} />
              <Tooltip contentStyle={RECHARTS_TOOLTIP} />
              <Line type="monotone" dataKey="opened" stroke="#C53030" strokeWidth={2} dot={false} name="Opened" />
              <Line type="monotone" dataKey="closed" stroke="#3DAA8A" strokeWidth={2} dot={false} name="Closed" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#1A3648]/60 border border-[#2D6A8F]/20 rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <ProgressRing value={latest?.vulnerability_count === 0 ? 100 : Math.max(100 - (latest?.vulnerability_count || 0) * 20, 0)} label="Security" sublabel={`${latest?.vulnerability_count ?? 0} vulns`} color={latest?.vulnerability_count > 0 ? '#C53030' : undefined} />
        </div>
      </div>
        ...
        <div className="bg-[#1A3648]/60 border border-[#2D6A8F]/20 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={trend}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2D6A8F" opacity={0.2} />
              <XAxis dataKey="date" stroke="#4A6E7F" tick={{ fill: '#6B8A9A', fontSize: 10 }} />
              <YAxis stroke="#4A6E7F" tick={{ fill: '#6B8A9A', fontSize: 10 }} />
              <Tooltip contentStyle={RECHARTS_TOOLTIP} />
              <Line type="monotone" dataKey="test_coverage_pct" stroke="#3DAA8A" strokeWidth={2} dot={false} name="Coverage %" />
              <Line type="monotone" dataKey="lint_errors" stroke="#F5A623" strokeWidth={2} dot={false} name="Lint Errors" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {latest?.bundle_size_kb && (
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#1A3648]/60 border border-[#2D6A8F]/20 rounded-xl p-4">
            <div className="text-xs text-[#6B8A9A] mb-1">Bundle Size</div>
            <div className="text-lg font-bold text-[#D0E4EC]">{(latest.bundle_size_kb / 1024).toFixed(1)} MB</div>
          </div>
          <div className="bg-[#1A3648]/60 border border-[#2D6A8F]/20 rounded-xl p-4">
            <div className="text-xs text-[#6B8A9A] mb-1">Last Updated</div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap ${
                activeSprint?.id === s.id
                  ? 'bg-[#3DAA8A]/20 text-[#3DAA8A] border border-[#3DAA8A]/30'
                  : 'bg-[#1E3A4A]/60 text-[#6B8A9A] border border-[#2D6A8F]/15 hover:border-[#2D6A8F]/30'
              }`}
            >
              {s.name}
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 bg-[#3DAA8A] hover:bg-[#3DAA8A]/80 rounded-lg text-xs font-medium text-white transition-colors"
        >
      ...
      {/* Create Sprint */}
      {showCreate && (
        <div className="bg-[#1A3648]/60 border border-[#2D6A8F]/20 rounded-xl p-4 flex gap-3 items-end">
          <div className="flex-1">
            <label className="text-xs text-[#6B8A9A] mb-1 block">Sprint Name</label>
            <input
              value={newSprintName}
              onChange={(e) => setNewSprintName(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleCreateSprint()}
              placeholder="e.g., Sprint 12 - Auth Fixes"
              className="w-full bg-[#1E3A4A] border border-[#2D6A8F]/20 rounded-lg px-3 py-2 text-sm text-[#D0E4EC] placeholder-[#4A6E7F] outline-none focus:border-[#3DAA8A]/40"
            />
          </div>
          <button onClick={handleCreateSprint} disabled={creating} className="px-4 py-2 bg-[#3DAA8A] rounded-lg text-sm text-white disabled:opacity-50">
      ...
      {activeSprint ? (
        ...
        <div className="text-center py-12 text-sm text-[#6B8A9A]">
